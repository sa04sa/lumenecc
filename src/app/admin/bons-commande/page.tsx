...
export default async function BonsCommandePage() {
  const factures = (await getDocuments('bon_commande')) as any[];
    ...
    <div className="space-y-8 text-slate-900">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">{factures.length} bon{factures.length > 1 ? 's' : ''} de commande</p>
        </div>
        <Link
          href="/admin/bons-commande/nouveau"
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {factures.length > 0 ? (
                  ...
                  <tr key={f.id} className="hover:bg-blue-50/40 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-1">
                        {!isDraft && !f.has_validated_children && (
                          <Link
                            href={`/admin/bons-livraison/nouveau?from=${f.id}&fromType=bon_commande`}
                        ...
                        {!isDraft && f.has_validated_children && null}
                        <Link
                          href={`/admin/bons-commande/${f.id}`}
                          className={`p-2 rounded-lg transition-colors ${
                            isDraft 
                              ? "text-blue-600 hover:bg-blue-50" 
                              : "text-slate-500 hover:bg-slate-100"
                          }`}
                          title={isDraft ? "Éditer le brouillon" : "Consulter le bon de commande"}
                        >
          ...
          <div className="py-24 text-center">
            ...
            <h3 className="text-slate-800 font-bold text-lg mb-1">Aucun bon de commande</h3>
            <p className="text-slate-500 mb-6">Créez votre premier bon de commande dès maintenant</p>
            <Link
              href="/admin/bons-commande/nouveau"
