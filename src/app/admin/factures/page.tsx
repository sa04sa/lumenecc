import { getFactures, getParametres } from "../actions";
import Link from "next/link";
import { Plus, FileText, Clock, CheckCircle2, Edit, Eye } from "lucide-react";
import PDFButton from "@/components/admin/PDFButton";

export default async function FacturesPage() {
  const factures = (await getFactures()) as any[];
    ...
    <div className="space-y-8 text-slate-900">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">{factures.length} facture{factures.length > 1 ? 's' : ''}</p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {factures.length > 0 ? (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center">
                        <Link
                          ...
                          title={isDraft ? "Éditer le brouillon" : "Consulter la facture"}
                        >
          ...
          <div className="py-24 text-center">
            ...
            <h3 className="text-slate-800 font-bold text-lg mb-1">Aucune facture</h3>
            <p className="text-slate-500 mb-6">Créez votre première facture dès maintenant</p>
            <Link
              href="/admin/factures/nouvelle"
