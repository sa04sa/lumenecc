import { getClients, getParametres, getDocumentById, getFactureLignes, getDocumentChildren, getDeliveryProgress } from "../../actions";
import InvoiceForm from "../../factures/nouvelle/InvoiceForm";
import DocumentActions from "@/components/admin/DocumentActions";
import { notFound } from "next/navigation";

export default async function EditBonCommandePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const progress = await getDeliveryProgress(factureId);

  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-900">
          {facture.statut === 'validee' ? 'Consulter le Bon de Commande' : 'Modifier le Brouillon'}
        </h1>
